...
st.markdown(
    ...
    .app-title { font-size: 2rem; font-weight: 700; color: #1E3A8A; margin-bottom: 0.5rem; }
    .app-subtitle { color: #6b7280; margin-top: 0.2rem; font-size: 1rem; }
    .assistant-bubble { background: linear-gradient(135deg, #3B82F6 0%, #1E3A8A 100%); color: white; padding: 1rem 1.2rem; border-radius: 16px; margin-bottom: 1rem; box-shadow: 0 2px 8px rgba(30, 58, 138, 0.2); }
    .user-bubble { background: #F3F4F6; color: #111827; padding: 1rem 1.2rem; border-radius: 16px; margin-bottom: 1rem; border: 1px solid #E5E7EB; }
    .stButton > button { background: #3B82F6; color: white; border: none; border-radius: 8px; padding: 0.5rem 1.5rem; font-weight: 600; transition: all 0.3s; }
    .stButton > button:hover { background: #1E3A8A; box-shadow: 0 4px 12px rgba(30, 58, 138, 0.3); }
    .stTextInput > div > div > input { border-radius: 8px; border: 2px solid #E5E7EB; padding: 0.75rem; }
    .stTextInput > div > div > input:focus { border-color: #3B82F6; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1); }
    [data-testid="stSidebar"] { background: linear-gradient(180deg, #F9FAFB 0%, #F3F4F6 100%); }
    </style>
    """,
...
def process_special_commands(prompt: str) -> tuple[bool, str]:
    ...
    if "respostas da planilha" in prompt_lower or "respostas do planilha" in prompt_lower:
        try:
            sheets = list_spreadsheets()
            if not sheets:
...
Seja objetivo, profissional e forneça respostas em português brasileiro."""
        ...
        messages = [system_message] + st.session_state.history[-10:]
