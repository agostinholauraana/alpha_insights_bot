...
# Acesso defensivo a `st.secrets` para evitar KeyError em ambientes onde a chave não existe.
for _k in ("GOOGLE_SERVICE_ACCOUNT_JSON", "GOOGLE_SERVICE_ACCOUNT_FILE"):
    try:
        ...
        # Tenta obter via .get (mais seguro), fallback para indexação
        val = None
        try:
            if hasattr(secrets_obj, 'get'):
                val = secrets_obj.get(_k)  # type: ignore[arg-type]
            else:
                val = secrets_obj[_k]  # type: ignore[index]
        except Exception:
            val = None
        ...
        # grava sem expor private_key
        if isinstance(val, dict):
            os.environ[_k] = json.dumps(val, ensure_ascii=False)
        else:
            os.environ[_k] = str(val)
    except Exception:
        # não permitir que o app quebre na importação por causa de secrets
        continue
